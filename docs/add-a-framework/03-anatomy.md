...
| `commands.ts`                                                         | `CommandDescriptor[]` with `owner: '<id>'`, `surfaces`, `iconKey`, `telemetry`, the legend row each artefact subscribes (`legend`, `legendBox`), and the icon map.                                                                                                | gated (`CommandExtension`)          |
...
| `toolbar/config.ts`, `toolbar/node-config.ts`                         | Contextual toolbar modules for the board (export, resize toggle) and the artefact. Legend and Validation sit together in the ONE `custom:affine:surface:<board>` module a flavour may carry, which is gated.                                                      | render, plus one gated module       |
...
| _(no file)_                                | The legend is `legend` / `legendBox` on the commands plus `legendToolbarAction` in the gated toolbar module (ADR 0026). A `legend.ts` at most holds the row builders `commands.ts` derives from its presets. | gated (the button)             |
...
| `packages/affine/all/src/frameworks.ts`                                  | the descriptor: id, labelKey, iconKey, chordPrefix (if any), telemetryKey, bundle, pkg, dir, the two extensions                                                                 |
