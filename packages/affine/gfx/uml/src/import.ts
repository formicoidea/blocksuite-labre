...
function draftsOf(model: UmlModel): UmlDraft[] {
  ...
  for (const port of model.ports) {
    node(port, 'port', { name: umlNameTierText(port) });
  }
...
export function umlElementsFromModel(
  ...
  for (const model of models) {
    ...
    const invented = placeDrafts(drafts, model.relations, layout);
    if (invented > 0) {
      notes.push({
    ...
    for (const relation of model.relations) {
      const source = shapeOf.get(relation.sourceId);
      const target = shapeOf.get(relation.targetId);
...
function placeDrafts(
  ...
  for (const draft of drafts) {
    const hint = hinted?.[draft.sourceId];
    if (hint) draft.bounds = hint;
  }

  const loose = drafts.filter(draft => draft.bounds === undefined);
  ...
  const boxes = umlInventLayout(
    loose.map(draft => ({ id: draft.sourceId, size: draft.size })),
    relations
  );
