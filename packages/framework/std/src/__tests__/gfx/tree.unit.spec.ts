...
describe('tree utils', () => {
  test('batchAddChildren prefers container.addChildren and deduplicates', () => {
    ...
    batchAddChildren(container as never, [a, a, b]);
  ...
  test('batchRemoveChildren falls back to container.removeChild and deduplicates', () => {
    ...
    batchRemoveChildren(container as never, [a, a, b]);
  ...
  test('the batch helpers do nothing when handed nothing', () => {
    const container = {
      addChildren: vi.fn(),
      removeChildren: vi.fn(),
    };

    batchAddChildren(container as never, []);
    batchRemoveChildren(container as never, []);

    expect(container.addChildren).not.toHaveBeenCalled();
  ...
  test('descendantElementsImpl stops on cyclic graph', () => {
    ...
    const descendants = descendantElementsImpl(groupA as never);
  ...
  test('canSafeAddToContainer blocks self and circular descendants', () => {
    ...
    expect(canSafeAddToContainer(parent as never, parent as never)).toBe(false);
    expect(canSafeAddToContainer(child as never, parent as never)).toBe(false);
    expect(canSafeAddToContainer(parent as never, unrelated as never)).toBe(
      true
    );
  });
});
