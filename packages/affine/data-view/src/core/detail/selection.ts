...
export class DetailSelection {
  ...
  onSelect = (selection?: DetailViewSelection) => {
    ...
    // Publish the new selection before blurring: leaving editing mode runs the
    // cell's exit hook, which may write a value back and re-enter this setter.
    this._selection = selection;
    if (old) {
      this.blur(old);
    }
    // The blur above may have moved the selection somewhere else; only focus
    // when it is still the one we were asked for.
