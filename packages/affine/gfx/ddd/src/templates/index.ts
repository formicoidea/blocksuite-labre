...
import { CD_TEMPLATES, CM_TEMPLATES, ES_TEMPLATES } from './components';
...
/**
 * One Templates-panel section per senior button (mirrors Cynefin/Estuarine),
 * plus a standalone Aggregate Design Canvas section. These are gated by
 * `ddd-templates` only — never by a senior-button flag — so the catalogue
 * stays available even when a senior button is hidden.
 */
