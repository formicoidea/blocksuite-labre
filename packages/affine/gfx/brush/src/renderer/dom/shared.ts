...
/**
 * The nodes a stroke host already owns, kept across renders. Rebuilding the
 * `<svg>` on every frame allocated one element per stroke per pan or zoom;
 * the attributes are cheap to overwrite in place.
 */
...
/**
 * Paints a brush or highlighter stroke into its host element. The z-index is
 * owned by `DomRenderer`, which reads it from the layer manager.
 */
