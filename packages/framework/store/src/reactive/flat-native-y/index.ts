...
> {
  ...
  constructor(
    ...
    // Props declared with an `undefined` default are never written to the Y.Map,
    // so they have no counterpart in `source`. Give them a signal anyway, so
    // consumers can read and write `prop$` before the prop ever gets a value.
    if (defaultProps) {
    ...
    // Defined defaults are applied after initialization, so that they are
    // written through to the Y.Map exactly as before.
    if (defaultProps) {
      Object.entries(defaultProps).forEach(([key, value]) => {
