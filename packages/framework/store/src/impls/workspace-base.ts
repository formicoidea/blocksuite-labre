...
import { DocImpl } from './doc.js';
import { WorkspaceMetaImpl } from './workspace-meta.js';
...
/** @deprecated Kept for backward compatibility; use {@link WorkspaceOptions}. */
export type DocCollectionOptions = WorkspaceOptions;

/**
 * @internal
 * The self-contained {@link Workspace} implementation shared by the public
 * {@link WorkspaceImpl} and the test-only `TestWorkspace`. It owns its root
 * `Y.Doc` and wires `DocEngine`/`AwarenessEngine`/`BlobEngine` from the given
 * sources — defaulting to in-memory no-ops. Consumers that persist/sync do so
 * on the exposed `Y.Doc`s (see `.doc` and `getDoc(...).spaceDoc`), or by
 * passing real `docSources`/`blobSources`.
 *
 * Not part of any public entry point; import {@link WorkspaceImpl} instead.
 */
export class WorkspaceBase implements Workspace {
  ...
  readonly blockCollections = new Map<string, DocImpl>();
  ...
  getBlockCollection(docId: string): DocImpl | null {
    const space = this.docs.get(docId) as DocImpl | undefined;
    return space ?? null;
  }
