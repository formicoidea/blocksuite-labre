...
- da68dbb: Copy a link to the current document with Ctrl/Cmd+Alt+L, anchored on the selected blocks or canvas elements, ready to be pasted into a text as a reference — available wherever the host registers `GenerateDocUrlExtension`.
...
- d3253d3: Ctrl+wheel and trackpad pinch zoom the canvas, never the browser page, even while a popup, the validation bubble or a toolbar is under the pointer.
- fff6bea: The polygon tool shows a crosshair cursor and a marker at the pointer as soon as it is armed, before the first vertex.
